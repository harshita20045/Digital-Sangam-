...
export default function SubmitDialectPage() {
  ...
  const handleAudioUpload = (e) => {
    ...
    if (file) {
      if (!file.type.startsWith("audio/")) {
        showToast(
          "Please upload a valid audio file (MP3, WAV, AAC, etc.)",
          "error"
        );
        return;
      }
      ...
      setFormData((prev) => ({ ...prev, audioLink: file.name }));
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      word: formData.word,
      meaning: {
        hindi: formData.hindiMeaning,
        english: formData.englishMeaning,
      },
      ...
      examples: examples.map((ex) => ({
        ...
        exampleMeaning: {
          hindi: ex.exampleMeaning.hindi,
          english: ex.exampleMeaning.english,
        },
      })),
    };

    try {
      await axios.post(EndPoint.ADD_DIALECT, payload);
      showToast(
